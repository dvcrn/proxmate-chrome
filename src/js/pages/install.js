// Generated by CoffeeScript 1.7.1
(function() {
  'use strict';
  angular.module('proxmateApp', ['ngRoute', 'chrome']).config(function($locationProvider, $routeProvider) {
    $locationProvider.hashPrefix('!');
    return $routeProvider.when('/', {
      templateUrl: 'views/main.html',
      controller: 'MainCtrl'
    }).when('/install/:packageId', {
      templateUrl: 'views/install.html',
      controller: 'InstallCtrl'
    }).otherwise({
      redirectTo: '/'
    });
  });

  angular.module('proxmateApp').controller('MainCtrl', ['$scope', '$route', '$routeParams', function($scope) {}]);

  angular.module('proxmateApp').controller('InstallCtrl', [
    '$scope', 'Chrome', '$routeParams', function($scope, Chrome, $routeParams) {
      $scope.status = 'Installing...';
      return Chrome.installPackage($routeParams.packageId, function(response) {
        if (response.success) {
          $scope.status = 'Installed successfully!';
          return $scope.$digest();
        }
      });
    }
  ]);

}).call(this);
